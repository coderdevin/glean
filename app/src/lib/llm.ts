...
const AnalysisResponseSchema = z.object({
  ...
  // Free-form, self-growing taxonomy: the model proposes tags from content and
  // supplies each one's bilingual name + family. Kept as `unknown[]` here so a
  // malformed entry never fails the whole parse — sanitizeProposedTags (tags.ts)
  // is the sole validator, called by ingest.ts before the upsert.
  tags: z.array(z.unknown()).default([]),
  // Free-form category (self-growing taxonomy). Kept loose here so a malformed
  // value never fails the whole parse — sanitizeCategory (category.ts) is the
  // sole validator, called by ingest.ts before the upsert.
  category: z.unknown(),
...

{
  ...
  "tags": [ { "slug": "<小写-kebab-ascii>", "name_zh": "中文名", "name_en": "English Name", "family": "infra"|"data"|"code" } ],
  "category": { "slug": "<小写-kebab-ascii>", "name_zh": "中文名", "name_en": "English Name" },
  "score": 0.0-1.0,
  "subscores": {
    "novelty": 0.0-1.0,
    "depth": 0.0-1.0,
...
**category（自由提取，对象 \`{ slug, name_zh, name_en }\`）**：给这篇内容一个最贴切的主分类。
- **优先复用**输入里「已有 category」中契合的（直接用它的 slug）；只有都不贴切时才新建。
- slug 小写 kebab-case 纯 ascii（如 \`ai-agents\` / \`databases\` / \`web-infra\`），name_zh / name_en 是地道中英名。
- 取**领域级**粒度（一篇内容一个主分类），不要太细（细粒度交给 tags）。
- 参考粒度：infra（基础设施/网络/容器）、data（数据库/管线/存储）、code（语言/框架/工具链）、ai-agents、llm、security…—— 但不限于这些，按内容自由命名。

**tags（3-6 个）**：根据内容**自由提取**最贴切的主题标签，每个 tag 是一个对象 \`{ slug, name_zh, name_en, family }\`。
- **准确性第一**：每个标签都必须真实反映正文的核心主题，能在文中找到明确依据。**准确优先于数量**——贴切的不足 3 个时宁可少给，绝不为凑到 3-6 个而硬塞勉强相关的词。
- **优先复用**输入里「已有 tag」中契合的（直接用它的 slug，不要改写）；只有都不贴切时才**新建**。
- 新建时：slug 用小写 kebab-case 纯 ascii（\`vector-db\`，不是 \`Vector DB\` 或中文）；name_zh / name_en 是地道的中英显示名；family 三选一。
- 标签要**具体**到技术领域 / 概念（\`rag\` / \`vector-db\` / \`cli\` / \`mcp\` / \`compiler\`），不要只给最泛的横切词（\`performance\` / \`ai\`）。
- 建议含 1 个贴合 category 的领域标签 + 若干更具体的主题标签。
...
- **category（自由提取，对象 \`{ slug, name_zh, name_en }\`）**：给这个项目一个最贴切的领域级主分类。优先复用输入「已有 category」里契合的 slug；都不贴切才新建（slug 小写 kebab-case 纯 ascii，地道中英名）。参考：infra / data / code / ai-agents / llm / cli-tools…，但不限于这些，按项目自由命名。一篇一个主分类，细粒度交给 tags。
- **tags（3-6 个）**：根据项目内容**自由提取**最贴切的主题标签，每个是对象 \`{ slug, name_zh, name_en, family }\`。**准确性第一**——每个标签都要能在资料卡里找到依据，**准确优先于数量**，贴切的不足 3 个时宁可少给，绝不为凑数硬塞勉强相关的词。**优先复用**输入「已有 tag」里契合的 slug；都不贴切时才新建（slug 小写 kebab-case 纯 ascii，name_zh/name_en 地道双语名，family 三选一）。标签要具体到技术领域 / 概念（\`rag\` / \`vector-db\` / \`cli\` / \`mcp\`），别只给 \`performance\` / \`ai\` 这类泛词。
...
/** An existing tag shown to the model as a reuse hint (not a hard whitelist). */
...
export interface CallLlmAnalysisArgs extends CallLlmArgs {
  /** Existing tags the model should prefer to reuse before minting new ones.
   *  Soft hint, not a whitelist — the model may propose new tags freely. */
  taxonomy: TaxonomyTag[];
  /** Existing categories — same soft-reuse hint as taxonomy, for the
   *  self-growing category field. */
  categories: TaxonomyTag[];
}
  ...
  return callWithFallback(env, args, {
    ...
    buildMessage: () =>
      buildAnalysisUserMessage({
        ...
        categories: (args as CallLlmAnalysisArgs).categories,
...
async function callOnce<S extends z.ZodTypeAny>(
  ...
  const userMessage = cfg.phase === "analysis"
    ? buildAnalysisUserMessage({
        title: args.title,
        body: trimmedBody,
        taxonomy: (args as CallLlmAnalysisArgs).taxonomy,
        categories: (args as CallLlmAnalysisArgs).categories,
        sourceHost: args.sourceHost,
        submitterNote: args.submitterNote,
        submittedDate: args.submittedDate,
  ...
  if (cfg.phase === "analysis") {
    ...
    // category is free-form now — sanitizeCategory (in ingest) is the validator.
  }
