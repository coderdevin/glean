...
**tags（2-4 个）**：根据内容**自由提取**最贴切的主题标签，每个 tag 是一个对象 \`{ slug, name_zh, name_en, family }\`。
...
- 标签要**具体**到技术领域 / 概念（\`rag\` / \`vector-db\` / \`cli\` / \`mcp\` / \`compiler\`），不要只给最泛的横切词（\`performance\` / \`ai\`）凑数；宁可少给，不要硬凑。
- 建议含 1 个贴合 category 的领域标签 + 1-3 个更具体的主题标签。
...
- **tags（2-4 个）**：根据项目内容**自由提取**最贴切的主题标签，每个是对象 \`{ slug, name_zh, name_en, family }\`。**优先复用**输入「已有 tag」里契合的 slug；都不贴切时才新建（slug 小写 kebab-case 纯 ascii，name_zh/name_en 地道双语名，family 三选一）。标签要具体到技术领域 / 概念（\`rag\` / \`vector-db\` / \`cli\` / \`mcp\`），别只给 \`performance\` / \`ai\` 这类泛词凑数；宁缺毋滥。
